...
@Component("WaitPoint")
public class WaitPointTaskHandler extends AbstractTaskHandler {
	...
	public void processTask(long workItemId, String workItemName, Map<String, Object> verifiedParameters, Map<String, Object> results) throws Exception {
		...
		if (workItem == null) {
			throw new Exception("Work item is null in wait point task handler.");
		}

		ProcessInstance processInstance = sessionManager.getSession().getProcessInstance(workItem.getProcessInstanceId());
		if (!(processInstance instanceof WorkflowProcessInstance)) {
			throw new Exception ("ProcessInstance is not WorkflowProcessInstance in wait point task handler.");
		}
		...
		if (existingWaitPointWorkItemId != null) {
			...
			waitPointMap.put(signal, workItemId);
		}
